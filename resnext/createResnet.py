...
class ResNetCreator(object):
    # constructor
    def __init__(self, blockDepth = 3):
        self.blockDepth = blockDepth
        if self.blockDepth == 2:
            self.resnetString = "n.res(n)_bn1, n.res(n)_relu1, n.res(n)_conv1, \
                        n.res(n)_bn2, n.res(n)_relu2, n.res(n)_conv2, n.res(n)_add = \
                        self.resnetBlock2((bottom), output=(output))"
        else:
            self.resnetString = "n.res(n)_bn1, n.res(n)_relu1, n.res(n)_conv1, \
                        n.res(n)_bn2, n.res(n)_relu2, n.res(n)_conv2, \
                        n.res(n)_bn3, n.res(n)_relu3, n.res(n)_conv3, n.res(n)_add = \
                        self.resnetBlock3((bottom), output=(output))"

    # interface
    def create(self, dataLayer, dataLayerParams, stages=[3, 4, 6, 3], classCount = 2, phase = "train"):
        n = caffe.NetSpec()

        n.data, n.label = L.Python(module="NpyDataLayer", layer=dataLayer, ntop=2, param_str=str(dataLayerParams))
    ...
    # helper
    def resnetBlock2(bottom, numOfOutput, kernelSize = 3, stride = 1, pad = 1):
        bn1 = L.BatchNorm(bottom, use_global_stats=False, in_place=True)
        relu1 = L.ReLU(bn1, in_place=True)
        conv1 = L.Convolution(relu1, num_output=numOfOutput, kernel_size=kernelSize, stride=stride, pad=pad, bias_term=False,
                              param=[dict(lr_mult=1, decay_mult=1)], weight_filler=dict(type="xavier"))

        bn2 = L.BatchNorm(conv1, use_global_stats=False, in_place=True)
        relu2 = L.ReLU(bn2, in_place=True)
        conv2 = L.Convolution(relu2, num_output=numOfOutput, kernel_size=kernelSize, stride=stride, pad=pad, bias_term=False,
                              param=[dict(lr_mult=1, decay_mult=1)], weight_filler=dict(type="xavier"))
    ...
    def resnetBlock2(bottom, output):
        bn1 = L.BatchNorm(bottom, use_global_stats=False, in_place=True)
        relu1 = L.ReLU(bn1, in_place=True)
        conv1 = L.Convolution(relu1, num_output=numOfOutput, kernel_size=kernelSize, stride=stride, pad=pad, bias_term=False,
                              param=[dict(lr_mult=1, decay_mult=1)], weight_filler=dict(type="xavier"))
        ...
        conv3 = L.Convolution(relu3, num_output=numOfOutput, kernel_size=kernelSize, stride=stride, pad=pad, bias_term=False,
                              param=[dict(lr_mult=1, decay_mult=1)], weight_filler=dict(type="xavier"))
